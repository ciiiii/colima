...
func (d dockerRuntime) setupInVM() error {
	// install in VM
	err := d.guest.Run("sudo", "sed", "-i", "s@http://.*archive.ubuntu.com@http://mirrors.ustc.edu.cn@g", "/etc/apt/sources.list")
	if err != nil {
		return fmt.Errorf("error installing in VM: %w", err)
	}
	err = d.guest.Run("sudo", "sed", "-i", "s@http://.*security.ubuntu.com@http://mirrors.ustc.edu.cn@g", "/etc/apt/sources.list")
	if err != nil {
		return fmt.Errorf("error installing in VM: %w", err)
	}
	...
	err = d.guest.Run("bash", "-c", "curl -fsSL https://mirrors.ustc.edu.cn/docker-ce/linux/ubuntu/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg")
	if err != nil {
		return fmt.Errorf("error installing in VM: %w", err)
	}
	err = d.guest.Run("bash", "-c", "echo \"deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://mirrors.ustc.edu.cn/docker-ce/linux/ubuntu $(lsb_release -cs) stable\" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null")
	if err != nil {
		return fmt.Errorf("error installing in VM: %w", err)
	}
	err = d.guest.Run("sudo", "apt-get", "update")
